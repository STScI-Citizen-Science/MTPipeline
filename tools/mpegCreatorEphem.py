...
def buildMovies(movieType, path):
    ...
    output = os.path.join(ROOTPATH, "movies", "temp", source + "All" + movieType + "_ephem.mp4") 
    subprocess.call(['ffmpeg', '-f', 'image2', '-r', '1',
                    '-pattern_type', 'glob', '-i','*'+ movieType + '*linear_ephem.png',
                    output])
    #for the comsmic ray rejected images   
    output = os.path.join(ROOTPATH, "movies", "temp", source + "CR" + movieType + "_ephem.mp4") 
    subprocess.call(['ffmpeg', '-f', 'image2', '-r', '1',
                    '-pattern_type', 'glob', '-i', '*cr*' + movieType + '*linear_ephem.png',
                    output])
    #for the non-cosmic ray rejected images
    output = os.path.join(ROOTPATH, "movies", "temp", source + "nonCR" + movieType + "_ephem.mp4") 
    #make a list of non-CR rejected images
    nonCRWideInput = [i for i in listDir if movieType in i and'linear_ephem' in i and 'cr' not in i]
